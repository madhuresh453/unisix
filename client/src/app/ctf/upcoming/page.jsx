              ...
              <article key={event.id} className="group rounded-xl border border-white/10 bg-[#080808] p-5 transition-all duration-300 hover:-translate-y-1 hover:border-[#ff1f45]/60 hover:shadow-[0_0_22px_rgba(255,0,60,0.22)]">
                ...
                <div className="mt-4 flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm text-white/70">{event.description.slice(0, 80)}...</p>
                  </div>
                </div>
                <div className="mt-4 flex flex-wrap gap-4 text-sm text-white/70">
                  <span className="flex items-center gap-1"><Clock3 className="h-4 w-4 text-[#ff1f45]" /> {event.duration}</span>
                  <span className="flex items-center gap-1"><Users className="h-4 w-4 text-[#ff1f45]" /> {event.players}</span>
                  <span className="flex items-center gap-1"><Activity className="h-4 w-4 text-[#ff1f45]" /> {event.difficulty}</span>
                </div>
                <div className="mt-5 flex gap-3">
                  <Link href={`/ctf/${event.id}/register`} className="inline-flex items-center gap-2 rounded-md bg-[#ffcf00] px-4 py-2 text-xs font-semibold uppercase tracking-[0.08em] text-black transition-all duration-300 hover:brightness-105">Register Now <ChevronRight className="h-4 w-4" /></Link>
                  <Link href={`/ctf/${event.id}`} className="inline-flex items-center rounded-md border border-white/20 px-4 py-2 text-xs font-semibold uppercase tracking-[0.08em] text-white transition-all duration-300 hover:border-[#ff1f45]/70">View Details</Link>
                </div>
              </article>
            ))}
          </div>
